...
if __name__ == "__main__":
    ...
    a = np.random.uniform(2, 4 , (2,50))
    l_a = np.zeros(50) 
    b = np.random.uniform(5, 8 , (2,50))
    l_b = np.ones(50)  

    c = np.random.uniform(9, 11 , (2,50))
    l_c = np.ones(50) + 1
    
    d = np.random.uniform(9, 11 , (2,50))
    d[0] -= 3
    l_d = np.ones(50) + 2


    data = np.hstack((a,b, c, d))
    labels = np.concatenate((l_a, l_b, l_c, l_d))
    ...
    #    plt.plot(data.T[ind].T[0], data.T[ind].T[1], "o")
    #plt.show()
    tree = Decision_tree(data, labels)
    tree.train()

    '''
    c = tree.classify(data)


    for lab in np.unique(c):
        ind = np.where(c == lab)
        plt.plot(data.T[ind].T[0], data.T[ind].T[1], "o", label = lab)
    plt.legend()
    plt.show()  
    '''
    ...
    print('-----')
    print_tree_structure(tree, ['depth', 'labels.shape'])
